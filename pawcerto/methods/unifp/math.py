...
# Supported tensor replacements for the Isaac Gym quaternion helpers. xyzw order.
def quat_apply(q, v):
    xyz = q[..., :3]
...
# Isaac Gym's helper returns a tuple with angles modulo 2 pi; the upstream
# utility above instead returns a stacked signed tensor. Preserve both APIs.
get_euler_xyz_signed_tensor = get_euler_xyz


def get_euler_xyz(q):
    angles = get_euler_xyz_signed_tensor(q)
    return tuple(angles[:, i] % (2 * np.pi) for i in range(3))
