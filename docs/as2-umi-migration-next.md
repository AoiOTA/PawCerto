...
本方案保留原4000-update候选的CPU参数审查、case11诊断和冻结动作执行器比较。后续已完成[带导轨/转接板的有限资产族与消费者](as2-pretraining-family.md)，以及用户批准的旧AS2 actor适配：1024环境、1000个新更新，用时55.37分钟，八组最终对照全部报告。名义Lab跟踪及支撑退化，四组Mu均数值失败，未建立可用迁移控制；原4000-update实验保持独立。随后22资产通用EE候选已完成1000次新训练及64例冻结评估，名义Lab存在局部臂/前移行为，但满载稳定和MuJoCo迁移仍未通过；见[最新结果](as2-general-ee-learning-result.md)。本页旧提案中的待训练描述保留其历史阶段含义，不能代替最新状态。
...
`outputs/as2-control-response-20260913/`已执行有界CPU协议，实际exit0、4848物理步/24.24模拟秒（上限8448步/42.24秒），无actor/optimizer或参数搜索。两资产各3秒零动作都有限、无warning，末0.5秒四脚地面支撑、无倒立或持续非足触地。名义末段root高0.29725–0.29867 m，满足预先声明的响应初态条件；满载为0.28580–0.29585 m，但末两个0.25秒窗口部分关节速度RMS增长（Piper2为0.16462→0.49032 rad/s），按协议跳过满载36组脉冲。该停止表示不满足这次响应协议的初态条件，不能称3秒数值失败或证明未来必然发散。

名义18关节×±0.01 rad的36条0.5秒脉冲全部完成；每分支精确恢复完整`mjSTATE_INTEGRATION`及runtime动作缓存，保留20 ms动作/延迟和5 ms PD。各步四脚支撑、无力矩限幅/位置越界/warning，峰值|qd|为0.279114 rad/s。匹配正负分支半差的目标关节响应峰为0.00688–0.01244 rad，最大末残余0.004156 rad，Piper2到Piper5耦合峰0.007903 rad；原始分支仍有基线漂移，不能以半差代替完整站姿验收。

另将相同五条冻结动作统一乘0.0008249437352，使最大目标偏移为0.01 rad，两资产各完成原初态0.12秒回放，无接触/限位/限幅/warning。腕6峰速名义0.182872、满载0.151999 rad/s；满载对应原幅度在既有回放达731.65 rad/s。该受控比较支持局部动作幅度依赖，但短窗口仍在腾空、零地面支撑，不是站立或策略验证；没有重复原幅度试验。当前没有据此更改增益、动作裁剪或学习配方，完整逐步和逐关节响应见该目录`result.md`、`comparison.json`及原始产物。
