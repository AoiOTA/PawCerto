...
本方案保留原4000-update候选的CPU参数审查、case11诊断和冻结动作执行器比较。后续已完成[带导轨/转接板的有限资产族与消费者](as2-pretraining-family.md)，并于2026-09-13按用户批准预算启动一次旧AS2 actor适配：1024环境，最多1000个新更新或90分钟训练循环时间。该次学习和最终对照由原执行负责人持有，结果尚待交付；原4000-update实验保持独立。

用户进一步明确的主路线是：**demo预训练阶段的质量、腿长、臂质量及COM分布应以AS2＋Piper-H为依据，并涵盖满载伸展带来的整机COM变化；随后冻结评估，必要时做有界微调。** 当前运行的六条件旧actor适配只是其中一个有限实验，不能替代完整参数分布及实际状态覆盖的证明。
