...
本方案记录CPU参数审查、case11诊断和冻结动作下的执行器约束比较。后续已实现[有限资产族与预训练消费者](as2-pretraining-family.md)，并验证参数实际加载；尚未开始新的学习。原单候选4000-update边界已经到达，新训练预算另行确定。默认依赖仍为官方、未修改的Isaac Lab / PhysX。

实际硬件装配须先修正为背部导轨、转接件与Piper-H底座的真实关系。当前0.12 m直接安装仍是历史仿真假设；官方背轨尺寸及尚缺的坐标/转接件参数见[装配依据](as2-piper-assembly.md)。下述CPU COM和旧资产实验不能作为真实装配校准。
