"""Export a trusted UniFP checkpoint and compare standalone CPU inference."""
import argparse
import json
...
def main(argv=None):
    ...
    parser.add_argument('--config', type=Path, help='Full env/train JSON for checkpoints without embedded config')
    ...
    import numpy as np
    import torch
    from pawcerto.methods.unifp.export import export_policy, load_checkpoint
    ...
    print(json.dumps(export_policy(loaded, args.output, observations), indent=2, allow_nan=False))
