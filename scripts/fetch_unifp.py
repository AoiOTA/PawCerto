#!/usr/bin/env python3
"""Fetch the fixed UniFP source and official B2/Z1 assets.
...
SHA = "68847a070f88d731058c3d8476929bc3b205f5bd"
REPO = "unified-force/UniFP"
...
def selected(path):
    return (path in ('LICENSE', 'README.md', 'setup.py')
            or path.startswith(('legged_gym/', 'resources/robots/b2z1/')))


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument('--destination', type=Path,
                        default=Path(__file__).resolve().parents[1] / 'third_party/unifp-reference')
    ...
    if not tree_path.exists():
        ...
        if tree.get('sha') != SHA or tree.get('truncated'):
            raise ValueError('Expected complete pinned UniFP source tree')
