...
Learning whole-body control for quadrupedal manipulators. PawCerto currently implements **UMI-on-Legs on the original Go2 + ARX5**: train with Isaac Lab / PhysX, evaluate fixed weights independently in MuJoCo, and export a CPU TorchScript actor with its execution configuration.
