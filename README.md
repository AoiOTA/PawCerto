...
Only UMI-on-Legs currently has completed learning and fixed-policy physical evaluation evidence. RoboDuet's original Go1 + ARX5 integration has actual Stage 1/2/resume, five-module export and fixed physical evaluations. Checkpoint 0 completes 2/9 frozen 20-second cases, while checkpoint 1600 completes 0/9; same-prefix leg tracking is worse at 1600. The released-arm early diagnostic does not isolate a Stage 1 training effect, and the 10001/50000 evaluations remain pending. DeepWBC on Go1 + WidowX 250s has completed bounded 21-update and fixed 500-step physical execution plus independent export consumption; this does not establish learning success. AS2 EDU + Piper H with a fixed-open stock gripper now has an active [4096-environment, 4000-update UMI candidate and evaluation protocol](docs/as2-umi-learning-plan.md), alongside the RoboDuet service. The shared-geometry OOM repair preserved all 601 nominal control rows exactly. The actual training-run model 0 completed 16/16 fixed MuJoCo cases, but mean tracking error was 0.51053 m / 1.63250 rad. The subsequent [model 500 evaluation](docs/as2-umi-model500-evaluation.md) had 13 numerical failures and three complete but inverted cases. Model 4000, final Lab comparison and export remain pending. Earlier inverted-pose and OOM attempts are retained. These incomplete integrations do not establish learned control, reliable transfer or real-hardware readiness. KISS My Agent supports development and is not a runtime dependency.

The approved [project roadmap](docs/project-plan.md) also includes UniFP and Learning Force Control for force/position compliance, Multi-critic Twist Tracking for velocity tracking and conflicting objectives once its materials are available, ReLIC as an optional research integration subject to its license restrictions, and LeggedManip_Lab as an engineering reference for multiple robot bodies. UniFP already has [original-source CPU integration](docs/unifp-implementation.md); its physical execution and the other expansion integrations follow the shared UMI/RoboDuet/DeepWBC and AS2/Piper H milestone. Completed UMI results keep their original experiment scope.
