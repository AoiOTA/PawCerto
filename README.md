...
A subsequent source-URDF comparison found incorrect principal inertia axes in the imported UMI USD. The converter now writes source-derived inertia tensors using official USD APIs. A separately identified corrected asset passed native dynamics checks and is being used for a new random-initialized 4000-update run on the fixed 71-train split. The older results above retain their original asset identity; the new run has no behavior-qualified result yet. See [the diagnosis and correction](docs/umi-contact-next-step.md).
