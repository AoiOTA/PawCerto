...
The [RoboDuet 50,000-update endpoint](docs/roboduet-model50000-evaluation.md) completed on 2026-09-14 with a negative physical result: all nine fixed cases terminated for low height within 0.10–0.28 seconds; exact export parity does not establish learned cooperation. A separate [AS2/Piper-H general-EE candidate](docs/as2-general-ee-pretraining.md) is now authorized for 1024 environments and at most 1000 updates or 90 training-loop minutes, followed by 64 fixed before/after cases. The initial progress snapshot was pushed before launch; the [AS2 endpoint](docs/as2-general-ee-learning-result.md) has now completed 1000 updates and all 64 cases after a documented recorder-only repair. Nominal Lab shows partial arm/forward behavior, but final nominal/loaded Lab have 3/8 and 7/8 inversions and final MuJoCo fails 16/16 numerically. Stable full-load control and cross-engine execution remain unproved.
